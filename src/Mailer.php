...
class Mailer
{
    ...
    public static function sendMail($mailConfig, $mailMessage,$isHtml = false)
    {
        ...
        $mail->Hostname = $mailConfig->getUsername();

        //设置发送的邮件的编码 可选GB2312 我喜欢utf-8 据说utf8在某些客户端收信下会乱码
        $mail->CharSet = 'UTF-8';

        //设置发件人姓名（昵称） 任意内容，显示在收件人邮件的发件人邮箱地址前的发件人姓名
        $mail->FromName = $mailConfig->getUsername();

        //smtp登录的账号 这里填入字符串格式的qq号即可
        $mail->Username = $mailConfig->getUsername();

        //smtp登录的密码 使用生成的授权码（就刚才叫你保存的最新的授权码）
        $mail->Password = $mailConfig->getPassword();

        //设置发件人邮箱地址 这里填入上述提到的“发件人邮箱”
        $mail->From = $mailConfig->getUsername();

        //邮件正文是否为html编码 注意此处是一个方法 不再是属性 true或false

        //设置收件人邮箱地址 该方法有两个参数 第一个参数为收件人邮箱地址 第二参数为给该地址设置的昵称 不同的邮箱系统会自动进行处理变动 这里第二个参数的意义不大
        foreach ($mailMessage->getRecevie() as $k => $v) {
            $mail->addAddress($v, $mailConfig->getUsername()); //添加收件人（地址，昵称）
